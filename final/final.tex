...
This grammar sketch describes the Amharic language of Ethiopia. Amharic is a Semitic language which is related to Arabic and Hebrew. It is mainly spoken in the Amhara region of Ethiopia and Addis Ababa, the capital. It has 25,600,000 speakers in Ethiopia, of which 21,600,000 are L1 users.\footnote{This information was retrieved from: Simons, Gary F. and Charles D. Fennig (eds.). 2017. Ethnologue: \textit{Languages of the World, Twentieth edition}. Dallas, Texas: SIL International. Online version: http://www.ethnologue.com.}

The language has a basic SOV word order, as shown in (\ref{ex:sov}).

\begin{exe}
  \ex\label{ex:sov} \gll (\orth{inE}) \orth{temarI} \orth{neny} \\
  I student be\textbackslash\textsc{1s} \\
  \trans `I am a student'
\end{exe}

Amharic is also a pro-drop language, and the pronoun \orth{inE} could be omitted from this sentence. Verbs agree with the person and number of the subject (and object in transitive verbs). All types of phrases are head-final, and the language features postpositions like in (\ref{ex:pp}).

\begin{exe}
  \ex\label{ex:pp} \gll \orth{inE} \orth{demena-w} \orth{lay} \orth{neber-kuwin} \\
  I cloud-\textsc{def.acc} on be\textbackslash\textsc{past}-\textsc{1s} \\
  `I was on the cloud'`
\end{exe}

Arguments are tracked through agreement, which is discussed in Section \ref{verbs}. However, the accusative case is marked in definite nouns, as can be seen in (\ref{ex:pp}) with the suffix \orth{-n}.
...
The plosives in Aharic are \ipa{p p' b t t' d k k' g P}. There is a three-way contrast for labial stops (voiced, voiceless, and ejective). All sounds occur word-initially, as can be seen in (1).
\begin{exe}
	\ex \ipa{pAsti} `pastry'
	\\ \ipa{bAdo} `empty'
        \\ \ipa{p'ap'as} `pope'
...
\noindent The ejective \ipa{p'} is rare in Amharic, but the alveolar and velar ejectives \ipa{t' k'} are quite common. Alveolar and velar stops have a three-way contrast between voiced, voiceless, and ejective variants. These can be seen in (2) and (3).
...
Amharic has at least two nasal consonant phonemes. \ipa{m} and \ipa{n} occur word-initially, as in (5). The palatal nasal only occurs word-medially and word-finally. 
\begin{exe}
  \ex \ipa{m1lAs} `tongue'
  \\ \ipa{n1fAs} `wind'
  \ex \ipa{AmAri\textltailn A} `Amharic'
\end{exe}
In the rest of the paper, the palatal nasal is represented with $<$ny$>$.
...
The front and mid high vowels \ipa{i} and \ipa{1} occur word-initially and word-finally in (15) and (16), but the back vowel \ipa{u} does not occur word-initially (and is shown word-finally in (15). However, the morpheme /-u/ is a definite marker in the language, so we have reason to believe that it is a phoneme.
...
\label{tab:pronouns}
  \begin{tabular}{lll}
    \textsc{1s} & \orth{inE} & I \\
    \textsc{2m}\footnote{The \textsc{2m, 2f, 3m}, and \textsc{3f} forms are all singular. This was shortened for convenience.} & \orth{ante} & you (m) \\
...
\begin{exe}
  ...
  you-\textsc{m} tall be\textbackslash\textsc{2m}\\
  \trans `You (m) are tall'

  \ex \gll \orth{anchI} \orth{rejim} \orth{nesh}\\
  you-\textsc{f} tall be\textbackslash\textsc{2f}\\
  \trans `You (f) are tall'

  \ex \gll \orth{isu} \orth{rejim} \orth{new}\\
  he tall be\textbackslash\textsc{3m}\\
  \trans `He is tall'

  \ex \gll \orth{iswa} \orth{rejim} \orth{nat}\\
  she tall be\textbackslash\textsc{3f}\\
  \trans `She is tall'
  ...
  \ex \gll \orth{inante} \orth{rejim} \orth{nachu}\\
...
Additionally, the language has demonstrative pronouns (like `this' and `that'). These pronouns are gendered, and the form of the copula agrees with the gender of the pronoun. In (\ref{ex:this}), we see that the verb \orth{new} `to be' is in the masculine form, which agrees with the masculine demonstrative pronoun \orth{ihE} `this'. In (\ref{ex:this-f}), \orth{nat}, the third person feminine form of the copula, agrees with the feminine demonstrative pronoun \orth{ichI} `this'.

\begin{exe}
  \ex\label{ex:this} \gll \orth{ihE} \orth{k'ey} \orth{wef} \orth{new} \\
  this red bird be\textbackslash\textsc{3m}\\
  \trans `This is a red bird'\footnote{In these glosses, \textsc{3f} means third person feminine singular, \textsc{3p} means third person plural, etc.} 

  \ex\label{ex:this-f} \gll \orth{ichI} \orth{gwadenya-ye} \orth{nat} \\
  this friend-\textsc{1s.poss} be\textbackslash\textsc{3f} \\
  \trans `This is my (female) friend.'
...
\noindent By changing the word order, these can also be used as determiners as in (\ref{ex:pronoun-det}).

\begin{exe}
  \ex\label{ex:pronoun-det} \gll \orth{inezI} \orth{wef-och} \orth{k'ey} \orth{nachew} \\
  these bird-\textsc{p} red be\textbackslash\textsc{3p}\\
  `These birds are red'
\end{exe}
...
In this pattern, the person and number of the possessor is shown by a suffix on the possessed noun. Second and third person singular suffixes distinguish between male and femaile, but the plural forms are not gendered.
...
Additionally, Amharic has several contrast forms for possession. These forms place the emphasis on the possessor and convey a meaning like `MY hand.' Each of these can also be expressed with a suffix form (without emphasis) as in the previous section. These are formed using the prefix \orth{ye-} before a pronoun (as listed in table \ref{tab:pronouns}).
...
\label{tab:contrast}
  \begin{tabular}{llll}
    \orth{ye-nE ij}      & `my hand'             & \orth{ij-E} & `my hand' \\
    \orth{y-ante ij}     & `your (m) hand'       & \orth{ij-ih} & `your (m) hand'\\
    \orth{y-anchI ij}    & `your (f) hand'       & \orth{ij-ish} & `your (f) hand'\\
    \orth{ye-su ij}      & `his hand'            & \orth{ij-u} & `his hand'\\
    \orth{ye-swa ij}     & `her hand'            & \orth{ij-wa} & `her hand'\\
    \orth{ye-nya ij-och} & `our hands'           & \orth{ij-och-achin} & `our hands'\\
    ?\orth{ye-nante ij-och}\footnote{This form was never elicited, but we did see \orth{yenante mets'af} `your (pl) book'.} & `your (pl) hands' & \orth{ij-och-achun} & `your (pl) hands'  \\ 
    \orth{ye-nesu ij-och} & `their hands' & \orth{ij-och-achew} & `their hands' \\
 \end{tabular}
\end{table}
...
\begin{exe}
  \ex\label{ex:possession_female} \orth{ye Rachel ij} `Rachel's hand'
  \ex\label{ex:bad_possession} *\orth{Rachel ijwa} `Rachel's hand'
  \ex\label{ex:possession_male} \orth{ye Abe ij} `Abe's hand'
\end{exe}
...
Reflexives are formed with the noun \orth{iras}, which may be derived from \orth{ras} `head.' \orth{iras} is the object of the sentence and takes a possessive suffix and a \orth{-n/-ny} object agreement suffix. In (\ref{ex:reflexive:wash}), we see the paradigm for the past tense of the verb \orth{metat'eb} `to wash.'
...
Each of these forms of \orth{iras} has an added \orth{-n/-ny/-in} object suffix unless the possessive suffix already ends in \orth{n}\footnote{This is probably the accusative definite suffix that occurs throughout the language, but it would require some more data to prove.}. The \orth{-ny} form occurs after the tense \orth{-E} suffix in the \textsc{1s} form, and the \orth{-in} form occurs after \orth{-ish}. This may be a result of the cluster \orth{*ishn} not being allowed in the language. (\ref{ex:reflexive:wash:gll1}) and (\ref{ex:reflexive:wash:gll2}) show the glosses of two of the forms.

\begin{exe}
  \ex\label{ex:reflexive:wash:gll1} \gll \orth{iras-E-ny} \orth{tat'eb-kuwin} \\
  \textsc{refl}-\textsc{1s.poss}-\textsc{acc} wash-\textsc{1s.s} \\
  \trans `I washed myself'

  \ex\label{ex:reflexive:wash:gll2} \gll \orth{iras-i-n} \orth{tat'eb-k} \\
  \textsc{refl}-\textsc{2m.poss}-\textsc{acc} wash-\textsc{2m.s} \\
  \trans `You (m) washed yourself'
\end{exe}

Reciprocals have a very similar paradigm. They use a different verbal suffix, \orth{te-} and a different verb root (above, \orth{tat'eb}, and here, \orth{tat'ib}). 

\begin{exe}
  \ex \orth{iras-achin te-tat'ib-en} `We washed each other'
  \ex \orth{iras-achun te-tat'ib-achu} `You (pl) washed each other'
  \ex \orth{iras-achew-n te-tat'ib-u} `They washed each other'
\end{exe}

The verb form for reflexives and reciprocals is as we expect from section 8.1 on subject person marking. The verb agrees with the subject of the sentence, which can be omitted in the case of prounouns. However, we don't see any object agreement.

In the present tense, we see the following paradigm in (\ref{ex:reflexive:wash-pres}):
\begin{exe}
  \ex\label{ex:reflexive:wash-pres} \orth{metat'eb} `to wash' \\
  \begin{tabular}{lll}
    \orth{iras-E-ny} & \orth{i-tat'ib-ale-w} & `I wash myself' \\
    \orth{iras-i-n} & \orth{ti-tat'ib-ale-h} & `You (m) wash yourself'\\
    \orth{iras-ish-in} & \orth{ti-tat'ib-yal-ish} & `You (f) wash yourself'\\
    \orth{iras-u-n} & \orth{ti-tat'ib-e}\footnote{Based on the present tense verb paradigm in Section \ref{verbs}, we would expect \orth{yI-tat'ib-ale}. This may need to be re-elicited from the speaker.} & `He washes himself'\\
    \orth{iras-wa-n} & \orth{ti-tat'ib-ale-ch} & `She washes herself'\\
    \orth{iras-achin} & \orth{ini-tat'ib-ale-n} & `We wash ourselves'\\
    \orth{iras-achun} & \orth{ti-tat'ib-ala-chu} & `You all wash yourselves'\\
    \orth{iras-achew-n} & \orth{yI-tat'ib-al-u} & `They wash themselves'
  \end{tabular}
\end{exe}

Two of these are glossed in (\ref{ex:reciprocal1}) and (\ref{ex:reciprocal2}).

\begin{exe}
  \ex\label{ex:reciprocal1} \gll \orth{iras-E-ny} \orth{i-tat'ib-ale-w} \\
  \textsc{refl-1s.poss-acc} \textsc{g1}-wash-\textsc{pres-1s} \\
  \trans `I wash myself'

  \ex\label{ex:reciprocal2} \gll \orth{iras-i-n} \orth{ti-tat'ib-ale-h} \\
  \textsc{refl-2m.poss-acc} \textsc{g2}-wash-\textsc{pres-2m} \\
  \trans `I wash myself'
\end{exe}

As in the past tense, a form of \orth{iras-} is followed by a verb in the present tense. This verb agrees with the person and number of the subject as we would expect according to the present tense verb paradigm in Section \ref{verbs}.
...
\begin{exe}
  ...
  \trans `The birds are small'\footnote{In these glosses, \textsc{3f} means third person singular feminine, \textsc{p} means plural, etc.}

  \iffalse
  \ex \gll \orth{ichI} \orth{wef} \orth{tinishIyE} \orth{nat} \\
  this bird small be\textbackslash\textsc{3f} \\
  \trans `This bird is small'
  \fi

  \ex\label{ex:cop-n} \gll \orth{Arsema} \orth{temarI} \orth{nech} \\
  Arsema student be\textbackslash\textsc{3f} \\
...
\begin{exe}
  ...
  \Tree [.S [.DP [.D \orth{ihE}\\this ] [.NP [.N \orth{wef}\\bird ] ] ] [.VP [.AdjP [.Adj \orth{k'ey}\\red ] ] [.V \orth{new}\\be\textbackslash\textsc{3m} ] ] ]

  \centering
  `This bird is red'

  \ex\label{ex:cop-order-n} 
  \Tree [.S [.NP [.N \orth{ihE}\\this ] ] [.VP [.NP [.AdjP [.Adj \orth{k'ey}\\red ] ] [.N \orth{wef}\\bird ] ] [.V \orth{new}\\be\textbackslash\textsc{3m} ] ] ]
...
Table \ref{tab:copula} lists the forms of the copula in the postive and negative. Interestingly, the forms of `to be' (possibly with the root \orth{ne}) match the suffixes for object prefixes, which can be seen in section 8.2. It may be possible to analyze these as \orth{ne-ny}, \orth{ne-h}, etc.
...
\label{tab:copula}
  \begin{tabular}{l|llll}
    ...
    \textsc{1s}  & \orth{neny} & `I am'          & \orth{ay-dele-hu-m} & `I am not'\\
    \textsc{2m} & \orth{neh} & `You (m) are'    & \orth{ay-dele-h-im} & `You (m) are not'\\
    \textsc{2f} & \orth{nesh} & `You (f) are'   & \orth{ay-dele-sh-im} & `You (f) are not'\\
    \textsc{3m} & \orth{new} & `He is'          & \orth{ay-dele-m} & `He is not'\\
    \textsc{3f} & \orth{nech/n-at} & `She is'   & \orth{ay-dele-ch-im} & `She is not'\\
    \textsc{1p}  & \orth{nen} & `We are'         & \orth{ay-dele-n-im} & `We are not'\\
    \textsc{2p}  & \orth{nachu} & `You (pl) are' & \orth{ay-del-achu-m} & `You (pl) are not'\\
    \textsc{3p}  & \orth{nachew} & `They are'    & \orth{ay-del-u-m} & `They are not'\\
  \end{tabular}
\end{table}

The negative forms of the copula are similar to the negation paradigm presented in section 8.1. They are formed by a prefix \orth{ay-} on the negative copula \orth{dele} with the negation suffix \orth{-im}. These may be gossed like in (\ref{ex:cop-neg}).

\begin{exe}
  \ex\label{ex:cop-neg} \gll \orth{ay-dele-h-im} \\
  \textsc{neg}-be-\textsc{2m-neg} \\
  \trans `You (m) are not'
\end{exe}
...
\noindent By contrast, (\ref{ex:past:work}) shows the forms for a verb root ending in a vowel.
...
\begin{exe}
  ...
  \begin{tabular}{ll}
    ...
    \orth{ser-achu} `You (pl) worked' & \orth{al-ser-achu-m} `You (pl) didn't work' \\
    \orth{ser-u} `They worked' & \orth{al-ser-u-m} `They didn't work' \\
  \end{tabular}
\end{exe}

For both types of verbs, the past negative is formed with the \orth{al-} prefix and the \orth{-im} suffix. All V-final forms preserve the final root vowel with the exception of the third-person plural form \orth{seru} (and possibly the second person plural form; however, this already ends in \orth{a}). In this form, the final vowel is dropped and replaced with the \orth{-u} suffix; otherwise, it would be indistinguishable from the \textsc{3m} form. These two paradigms are summarized in table \ref{tab:past:subject-affixes}.

\begin{table}[ht!]
\centering
\caption{Subject affixes}
\label{tab:past:subject-affixes}
  \begin{tabular}{l|ll}
    form & C-final & V-final \\ \hline
    \textsc{1s}  & \orth{-ku}   & \orth{-w} \\ 
    \textsc{2m} & \orth{-k}    & \orth{-h} \\
    \textsc{2f} & \orth{-sh}   & \orth{-sh} \\
    \textsc{3m} & \orth{-e}    & \orth{-$\emptyset$} \\
    \textsc{3f} & \orth{-ech}  & \orth{-ch} \\
    \textsc{1p}  & \orth{-en}   & \orth{-n} \\
    \textsc{2p}  & \orth{-achu} & \orth{-achu} \\
...
\begin{exe}
  ...
  \begin{tabular}{ll}
    \orth{inE i-rot'-ale-w} `I run'              & \orth{inE al-rot'-im} `I don't run' \\
    \orth{ante ti-rot'-ale-h} `You (m) run'      & \orth{ante at-rot'-im} `You (m) don't run \\
    \orth{anchI ti-roch'-ale-sh} `You (f) run'   & \orth{anchI at-roch'-im} `You (f) don't run \\
    \orth{isu yI-rot'-ale} `He run'             & \orth{isu ay-rot'-im} `He doesn't run' \\
    \orth{iswa ti-rot'-ale-ch} `She run'         & \orth{iswa at-rot'-im} `She doesn't run' \\
    \orth{inya in-rot'-ale-n} `We run'           & \orth{inya an-rot'-im} `We don't run' \\
    \orth{inante ti-rot'-al-achu} `You (pl) run' & \orth{inante at-rot'-u-m} `You (pl) don't run' \\
    \orth{inesu yI-rot'-al-u} `They run'         & \orth{inesu ay-rot'-u-m} `They don't run' \\
  \end{tabular}
\end{exe}

In the positive forms, \orth{-ale} occurs after the verb root and represents the present tense. Then, the V-final subject marking suffixes from table \ref{tab:past:subject-affixes} occur after \orth{ale}. The four different prefixes separate the forms into four different groups, as shown in table \ref{tab:pres:affixes}. This grouping seems arbitrary; it puts all 2nd person forms in one group along with the 3rd person feminine (group 2), while other third person forms are grouped (group 3) and first person forms are not grouped (groups 1 and 4). The second person feminine form also has palatalization in the verb root, as seen in (\ref{ex:pres:run}).
...
\label{tab:pres:affixes}
  \begin{tabular}{l|lll}
    ...
    \textsc{1s}  & \orth{i-}    & \orth{al-} & 1 (\orth{i-}/\orth{al-})\\ 
    \textsc{2m} & \orth{ti-}   & \orth{at-} & 2 (\orth{ti-}/\orth{at-})\\
    \textsc{2f} & \orth{ti-}  & \orth{at-} + palatalization & 2 (\orth{ti-}/\orth{at-})\\
    \textsc{3m} & \orth{yI-}    & \orth{ay-} & 3 (\orth{yI-}/\orth{ay-})\\
    \textsc{3f} & \orth{ti-}  & \orth{at-} & 2 (\orth{ti-}/\orth{at-})\\
    \textsc{1p}  & \orth{in-}   & \orth{an-} & 4 (\orth{in-}/\orth{an-})\\
    \textsc{2p}  & \orth{ti-} & \orth{at- -u} & 2 (\orth{ti-}/\orth{at-})\\
    \textsc{3p}  & \orth{yI-}    & \orth{ay- -u} & 3 (\orth{yI-}/\orth{ay-}) 
  \end{tabular}
\end{table}
...
\begin{multicols}{2}
\begin{exe}
  \ex\label{ex:merot:pres:gloss1} \gll \orth{inE} \orth{i-rot'-ale-w} \\
  I \textsc{g1.pres}-run-\textsc{pres}-\textsc{1s} \\
  \trans `I run'

  \ex\label{ex:merot:pres:gloss2} \gll \orth{ante} \orth{ti-rot'-ale-h} \\
  \textsc{2m} \textsc{g2.pres}-run-\textsc{pres}-\textsc{2m} \\
  \trans `You run'
\end{exe}
\end{multicols}
...
\begin{exe}
  ...
  \textsc{2m} \textsc{g2.pres.neg}-run-\textsc{neg} \\
  \trans `You don't run'
\end{exe}
\end{multicols}

\noindent Interestingly, the person marking suffix is absent in most of these negative forms. Only the \textsc{2p} and \textsc{3p} forms have a \orth{-u} suffix which occurs before the \orth{-im} suffix, which may represent the plural.
...
Verbs also agree with objects using verbal affixes. These affixes vary between the past and the present/future tenses.
...
There are several phonological processes that change the surface forms of the affixes. For example, in (\ref{ex:makif:phon1}), the third-person masculine singular subject is dropped when the object affix begins with a vowel. Additionally, in (\ref{ex:makif:phon2}), the \orth{u} of the \textsc{3f} affix becomes \orth{w} before a vowel. We see that this does not happen after a consonant in (\ref{ex:makif:phon1}). The \textsc{3m} form becomes \orth{-t} only after \orth{u}.

\begin{multicols}{2}
\begin{exe}
  \ex\label{ex:makif:phon1} \gll \orth{ak'if-$\emptyset$-at} \\
  hug-\textsc{3m.s}-\textsc{3f.o} \\
  \trans `He hugged her'

  \ex\label{ex:makif:phon2} \gll \orth{ak'if-achw-at} \\
  hug-\textsc{2p.s}-\textsc{3f.o} \\
  \trans `You (pl) hugged her'
\end{exe}
\end{multicols}

These forms are summarized in table \ref{tab:person-marking}. There are slightly different forms when the suffixes occur after verbs and consonants. 

\begin{table}[!ht]
\centering
\caption{Object suffixes}
\label{tab:person-marking}
  \begin{tabular}{l|ll}
    form & V\textunderscore & C\textunderscore \\ \hline
    \textsc{1s}  & \orth{-ny}    & \orth{-iny} \\ 
    \textsc{2m}  & \orth{-h}     & \orth{-ih} \\
    \textsc{2f}  & \orth{-sh}    & \orth{-ish} \\
    \textsc{3m}  & \orth{-w, -t}/u\textunderscore     & \orth{-iw} \\
    \textsc{3f}  & \orth{-t}     & \orth{-at} \\
    \textsc{1p}  & \orth{-n}     & \orth{-in} \\
    \textsc{2p}  & \orth{-achu}  & \orth{-achu} \\
    \textsc{3p}  & \orth{-achew} & \orth{-achew} \\
  \end{tabular}
\end{table}

Additionally, we have evidence that suggests that indirect objects work similarly. In these examples, the direct object marker comes before the root, and the indirect object marker occurs as a verbal suffix. The verb agrees with the subject and the indirect object in (\ref{ex:mestet:indirect}).
...
The paradigm for object marking in the present tense is listed in (\ref{ex:makif:obj-pres}). Here, we can also see pronouns with the accusitive suffix \orth{-n}.

\begin{exe}
  \ex\label{ex:makif:obj-pres} \orth{mak'if} `to hug' \\
  \begin{tabular}{lll}
    \orth{iswa inE-n} & \orth{t-ak'if-Eny-ale-ch}  & `She hugs me' \\
    \orth{iswa ante-n}& \orth{t-ak'if-ih-ale-ch}   & `She hugs you (m)' \\
    \orth{iswa anchI-n}& \orth{t-ak'if-ish-ale-ch}  & `She hugs you (f)' \\
    \orth{iswa isu-n}& \orth{t-ak'if-ale-ch}   & `She hugs him' \\
    \orth{iswa iswa-n}& \orth{t-ak'if-at-ale-ch}    & `She hugs her' \\
    \orth{iswa inya-n}& \orth{t-ak'if-en-ale-ch}   & `She hugs us' \\
    \orth{iswa inante-n}& \orth{t-ak'if-achw-ale-ch}  & `She hugs you (pl)' \\
    \orth{iswa inesu-n}& \orth{t-ak'if-wachew-ale-ch} & `She hugs them' 
  \end{tabular}
\end{exe}

The order of the agreement markers changes in the present tense; the object agreement marker occurs before the subject agreement marker, which is the opposite order of the past tense. The object marker occurs before \orth{-ale-}, and the subject marker occurs after it. These verb forms have the following template:

\begin{center}
 (group prefix)-ROOT-obj-\orth{ale}-subj.
\end{center}

Two of these forms are glossed in (\ref{ex:makif:obj-pres-gll1}) and (\ref{ex:makif:obj-pres-gll2}). 

\begin{exe}
  \ex\label{ex:makif:obj-pres-gll1} \gll \orth{iswa} \orth{inE-n} \orth{t-ak'if-Eny-ale-ch} \\
  she me-\textsc{acc} \textsc{g2.pres}-hug-\textsc{1s.o-pres-3f.s}\\
  \trans `She hugs me'

  \ex\label{ex:makif:obj-pres-gll2} \gll \orth{iswa} \orth{isu-n} \orth{t-ak'if-$\emptyset$-ale-ch} \\
  she me-\textsc{acc} \textsc{g2.pres}-hug-\textsc{3m.o-pres-3f.s}\\
  \trans `She hugs him'
\end{exe}

The negative forms of these lose their subject agreement markers. Negation is then marked with same affixes as in Table \ref{tab:pres:affixes}. This paradigm is shown in (\ref{ex:makif:obj-pres-neg}), two of these are glossed in (\ref{ex:makif:obj-pres-gll1-neg}) and (\ref{ex:makif:obj-pres-gll2}).

\begin{exe}
  \ex\label{ex:makif:obj-pres-neg} \orth{mak'if} `to hug' \\
  \begin{tabular}{lll}
    \orth{iswa inE-n} &   \orth{at-ak'if-Eny-im}  & `She hugs me' \\
    \orth{iswa ante-n}&   \orth{at-ak'if-ih-im}   & `She hugs you (m)' \\
    \orth{iswa anchI-n}&  \orth{at-ak'if-ish-im}  & `She hugs you (f)' \\
    \orth{iswa isu-n}&    \orth{at-ak'if-ew-m}   & `She hugs him' \\
    \orth{iswa iswa-n}&   \orth{at-ak'if-at-im}    & `She hugs her' \\
    \orth{iswa inya-n}&   \orth{at-ak'if-en-im}   & `She hugs us' \\
    \orth{iswa inante-n}& \orth{at-ak'if-achu-m}  & `She hugs you (pl)' \\
    \orth{iswa inesu-n}&  \orth{at-ak'if-achew-im} & `She hugs them' 
  \end{tabular}
\end{exe}

The negative forms have the following template:

\begin{center}
  (neg. group prefix)-ROOT-obj-\orth{im}
\end{center}

\begin{exe}
  \ex\label{ex:makif:obj-pres-gll1-neg} \gll \orth{iswa} \orth{inE-n} \orth{at-ak'if-Eny-im} \\
  she me-\textsc{acc} \textsc{g2.pres.neg}-hug-\textsc{1s.o-neg}\\
  \trans `She doesn't hug me'

  \ex\label{ex:makif:obj-pres-gll2-neg} \gll \orth{iswa} \orth{isu-n} \orth{at-ak'if-ew-m} \\
  she me-\textsc{acc} \textsc{g2.pres.neg}-hug-\textsc{3m.o-neg}\\
  \trans `She doesn't hug him'
\end{exe}
...
\begin{exe}
  ...
  room-\textsc{1s.poss-obj} clean-\textsc{1s.s-3sm.o} \\
  \trans `I cleaned my room'

  \ex\label{ex:caus:clean2} \gll \orth{inat-E} \orth{kifl-E-n} \orth{as-ts'eda-ch-iny} \\
  mom-\textsc{1s.poss} room-\textsc{1s.poss-obj} \textsc{caus}-clean-\textsc{3f.s-1s.o} \\
...
\begin{exe}
  \ex\label{ex:caus:tree1} \Tree [.S [.NP $[$pro$]$ ] [.VP [.NP N\\\orth{kifl-E-n}\\room-\textsc{1s.poss-obj} ] V\\\orth{ats'eda-w-t}\\clean-\textsc{1s.s-3sm.o} ] ]

  \ex\label{ex:caus:tree2} \Tree [.S [.NP N\\\orth{inat-E}\\mom-\textsc{1s.poss} ] [.VP [.NP N\\\orth{kifl-E-n}\\room-\textsc{1s.poss-obj} ] V\\\orth{as-ts'eda-ch-iny}\\\textsc{caus}-clean-\textsc{3f.s-1s.o} ] ]
\end{exe}
...
\begin{exe}
  ...
  \textsc{comp}-\textsc{3m.comp.pres}-come\textbackslash\textsc{3m.past} said-\textsc{3m.s}-\textsc{1s.o}-al \\
  \trans `He said that he's coming'

  \ex\label{ex:comp:inde2} \gll \orth{temarI} \orth{inde-hon-ku} \orth{tenager-ku} \\
  student \textsc{comp}-be-\textsc{1s.s} said-\textsc{1s.s.past} \\
  \trans `I said that I was a student'
\end{exe}
...
\begin{exe}
  ...
  pretty \textsc{comp}-be-\textsc{3f} asked-\textsc{1s.s.past} \\
  \trans `I asked if she was pretty'
\end{exe}

In some present tense examples, these prefixes are accompanied by a variation of \orth{-mi-} and even a verbal suffix in the \textsc{2f}, \textsc{2p}, and \textsc{3p} forms. It is unclear when these are used, as they occur in forms like (\ref{ex:comp:inde1}) but not in forms like (\ref{ex:comp:inde2}). This paradigm is shown in Table \ref{tab:comp:forms}.\footnote{These groupings match those seen in Table \ref{tab:pres:affixes} for subject person marking. The only difference is that the \textsc{2p} form seems to be different from the other \textsc{g2} forms.}
...
\begin{tabular}{lll}
  ...
  \textsc{2m} & \orth{meblat inde-mit-felig} & `that you (m) want to eat' \\
  \textsc{2f} & \orth{meblat inde-mit-felig-I} & `that you (f) want to eat' \\
  \textsc{3m} & \orth{meblat inde-mI-felig} & `that he wants to eat' \\
  \textsc{3f} & \orth{meblat inde-mit-felig} & `that she wants to eat' \\
...
(\ref{ex:comp:inde:tree}) shows the structure of these sentences and the movement of the verb up to the complementizer \orth{inde-}. As in the previous examples, the CP is the object of the verb \orth{belonyal}.

\begin{exe}
  \ex\label{ex:comp:inde:tree} \Tree [.S 
  [.NP  N\\$[$pro$]$  ] 
  [.VP 
    [.CP 
      [.S 
        [.NP N\\$[$pro$]$ ] 
        [.VP \node{vi}V\\\orth{mImet'a} ] 
      ] 
      \node{vf}C\\\orth{inde-} 
    ]
    V\\\orth{belonyal} 
...
\anodecurve[br]{vi}[br]{vf}{0.4in}
\end{exe}

\subsubsection{Null complementizer}

For direct quotations, complement clauses are formed without an explicit complementizer. This can be seen in (\ref{ex:comp:null1}) and (\ref{ex:comp:null2}).
\begin{exe}
  \ex\label{ex:comp:null1} \gll \orth{i-met'-ale-w} \orth{belo-$\emptyset$-ny-al} \\
  \textsc{g1.pres}-come-\textsc{pres-1s} said-\textsc{3m.s}-\textsc{1s.o}-al \\
  ...
  pretty be\textbackslash\textsc{3f} asked-\textsc{1s} \\
...
It is impossible to tell if verb moves up to the C as in other dependent clauses; however, it seems unlikely that these complement clauses would work any differently.

\subsubsection{Infinitives}

Infinitives are formed with the verbal prefix \orth{me-} and have verb movement to the complementizer as with other prefix complementizers. This is shown in (\ref{ex:inf1}), (\ref{ex:inf2}), and (\ref{ex:inf3}).
...
\begin{exe}
  ...
  \textsc{inf}-eat \textsc{comp}-want told-\textsc{3m.s-1s.o} \\
...
%TODO: look over one more time

Relative clauses in Amharic are CPs that modify a head noun. They use the relativizer \orth{ye-}, which occurs as a verbal prefix. Since Amharic is a head-final language, relative clauses occur before the nouns they modify. 

\subsubsection{Relativized subject}

When the relativized noun phrase is a subject, we see a gap in the subject position of the relative clause. However, the verb still agrees with the missing subject. In (\ref{ex:rel:gll_subj}), the verb \orth{-sera-} agrees with the third person feminine subject \orth{sEtIyo} (which is outside of the clause) and the third person masculine object \orth{buna}.

\begin{exe}
  \ex\label{ex:rel:gll_subj} \gll \orth{buna} \orth{ye-mit-sera-w} \orth{sEtIyo} \orth{nat} \\
  coffee \textsc{rel}-\textsc{3f.comp.pres}-make-\textsc{3m.o} woman be\textbackslash\textsc{3f}\\
  `Who is the woman who makes coffee?'
\end{exe}

\noindent We can see the structure of this sentence in (\ref{ex:rel:tree_subj}). There is a gap at the subject of the embedded CP, and the verb moves up to the complementizer.
...
\subsubsection{Relativized object}

When the relativized noun phrase is a direct object, we see a gap in the object position of the relative clause. However, the verb still agrees with the person and number of the object, which is \orth{libs} `clothes' in (\ref{ex:rel:gll_obj}).

\begin{exe}
  \ex\label{ex:rel:gll_obj} \gll \orth{ye-lebes-ke-w} \orth{libs} \orth{arIf} \orth{new} \\
  \textsc{rel}-wear-\textsc{2m.s-3s.o} clothes nice be\textbackslash\textsc{3s} \\
  \trans `The clothing that you wear is nice.'
\end{exe}

\noindent In (\ref{ex:rel:gll_obj}), the verb \orth{-lebes-} agrees with the second person masculine subject and the third person singular object \orth{libs} `clothes'.

The structure of this sentence can be seen in (\ref{ex:rel:tree_obj}). There is a gap at the object of the embedded CP, and the verb moves up to the complementizer.
...
We can see another example of this in (\ref{ex:rel:gll_obj2}).

\begin{exe}
  \ex\label{ex:rel:gll_obj2} \gll \orth{ye-sera-w-t} \orth{buna} \orth{yet} \orth{new} \\
...
\begin{exe}
  \ex\label{ex:rel:tree_obj2} \Tree [.S 
    [.NP
      [.CP 
        [.S 
          [.NP N\\$[$pro$]$ ]
...
\begin{exe}
\ex\label{ex:appendix-start} \gll \orth{timirt} \orth{indEt} \orth{new?} \\
how school be\textbackslash\textsc{3m} \\
\trans `How is school?'

\ex \gll \orth{timirt} \orth{arIf} \orth{new}\\
school nice be\textbackslash\textsc{3m}\\
\trans `School is nice.'

\ex \gll \orth{ye-miti-wej-iw} \orth{astemarI} \orth{ale-h}? \\
\textsc{rel}-\textsc{g2.comp}-love-\textsc{3m.o} teacher exist\textbackslash\textsc{3m}\\
\trans `Do you have a favorite teacher?'

\ex \gll \orth{yenE} \orth{math} \orth{astemarI} \orth{des} \orth{yIlal}. \orth{hIsab} \orth{lay} \orth{bet'am} \orth{yI-reda-ny-al}\\
\textsc{1s.poss} math teacher great is\textbackslash\textsc{3m}. math on lots \textsc{g2.pres}-help-\textsc{1s.o-pres} \\
\trans `My math teacher is great. He helps me with math.'

\ex \gll \orth{kesat} \orth{min} \orth{t-adirgy-ale-sh} \\
afternoon what \textsc{g2.pres}-do-\textsc{pres-2f}\\
\trans `What do you do after school?'

\ex \gll \orth{kesat} \orth{lay} \orth{igir} \orth{kwas} \orth{i-ch'awet-ale-w}\\
afternoon in foot ball \textsc{g1.pres}-play-\textsc{pres-1s}\\
\trans `After school I play soccer.'

\ex\label{ex:appendix-end} \gll \orth{timirtibEt-ish} \orth{bet'am} \orth{des} \orth{yIlal} \\
school-\textsc{3f.poss} very great is\textbackslash\textsc{3m} \\
\trans `Your school is really great.'
\end{exe}
