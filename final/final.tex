...
\begin{exe}
  ...
  hug-\textsc{3m.subj}-\textsc{3f.obj} \\
  \trans `He hugged her'

  \ex\label{ex:makif:phon2} \gll \orth{ak'if-achw-at} \\
  hug-\textsc{2p.subj}-\textsc{3f.obj} \\
...
\begin{exe}
  ...
  mom-\textsc{1s.poss} room-\textsc{1s.poss-obj} \textsc{caus}-clean-\textsc{3f.subj-1s.obj} \\
...
\begin{exe}
  ...
  \ex\label{ex:caus:tree2} \Tree [.S [.NP N\\\orth{inat-E}\\mom-\textsc{1s.poss} ] [.VP [.NP N\\\orth{kifl-E-n}\\room-\textsc{1s.poss-obj} ] V\\\orth{as-ts'eda-ch-iny}\\\textsc{caus}-clean-\textsc{3f.subj-1s.obj} ] ]
\end{exe}
...
\begin{exe}
  ...
  \textsc{g1.pres}-come-\textsc{pres-1s} said-\textsc{3m.subj}-\textsc{1s.obj}-al \\
...
\begin{exe}
  ...
  \textsc{comp}-\textsc{3m.comp}-come\textbackslash\textsc{3m.past} said-\textsc{3m.subj}-\textsc{1s.obj}-al \\
...
\begin{exe}
  ...
  pretty \textsc{comp}-be-\textsc{3f} asked-\textsc{1s} \\
...
\begin{exe}
  ...
  \textsc{inf}-eat \textsc{comp}-want told-\textsc{3m.subj-1s.obj} \\
...
When the relativized noun phrase is a direct object, we see a gap in the object position of the relative clause. However, the verb still agrees with the person and number of the object, which is \orth{libs} `clothes' in (\ref{ex:rel:gll_subj}).
...
\noindent In (\ref{ex:rel:gll_subj}), the verb \orth{-lebes-} agrees with the second person masculine subject and the third person singular object \orth{libs} `clothes'.

The structure of this sentence can be seen in (\ref{ex:rel:tree_subj}). There is a gap at the object of the embedded CP, and the verb moves up to the complementizer.
...
We can see another example of this in (\ref{ex:rel:gll_obj}).
...
\textsc{comp}-\textsc{g2.comp}-love-\textsc{3m.o} teacher exist\textbackslash\textsc{3m}\\
...
\textsc{1s.poss} math teacher great is\textbackslash\textsc{3m}. math on lots \textsc{g2.pres}-help-\textsc{1s.obj-pres} \\
