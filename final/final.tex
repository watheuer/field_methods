...
\begin{exe}
  ...
  \textsc{refl}-\textsc{1s.poss}-\textsc{acc} wash-\textsc{1s} \\
  \trans `I washed myself'

  \ex\label{ex:reflexive:wash:gll2} \gll \orth{iras-i-n} \orth{tat'eb-k} \\
  \textsc{refl}-\textsc{2m.poss}-\textsc{acc} wash-\textsc{2m} \\
...
The verb form for reflexives and reciprocals is as we expect from section 8.1 on subject person marking. It agrees with the subject of the sentence, which either occurs before the object or only as a verbal suffix (as seen here). However, we don't see any object agreement.
...
For both types of verbs, the past negative is formed with the \orth{al-} prefix and the \orth{-im} suffix. All V-final forms preserve the final root vowel with the exception of the third-person plural form \orth{seru}. In this form, the final vowel is dropped and replaced with the \orth{-u} suffix; otherwise, it would be indistinguishable from the \textsc{3m} form. These two paradigms are summarized in table \ref{tab:past:subject-affixes}.
...
\noindent Interestingly, the person marking suffix is absent in most of these negative forms. Only the \textsc{2p} and \textsc{3p} forms have a \orth{-u} suffix which occurs before the \orth{-im} suffix. This may represent the plural.
...
These forms are summarized in table \ref{tab:person-marking}. There are slightly different forms when the suffixes occur after verbs and consonants. Additionally, the \textsc{3m} form is \orth{-t} after \orth{u}.
...
In the present tense, the order of the affixes changes. The paradigm is listed in (\ref{ex:makif:obj-pres}). Here, we can see pronouns with the accusitive suffix \orth{-n}.
...
% TODO: fix these trees, don't use $t$ as leaf node
