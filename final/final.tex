...
Reflexives are formed with the noun \orth{iras}, which may be derived from \orth{ras} `head.' \orth{iras} is the object of the sentence and takes a possessive suffix and a \orth{-n/-ny} object agreement suffix. In (\ref{ex:reflexive:wash}), we see the paradigm for the verb \orth{metat'eb} `to wash.'
...
\begin{exe}
  ...
  \textsc{refl}-\textsc{2m.poss}-\textsc{obj} wash-\textsc{2m} \\
  \trans `You (m) washed yourself'
\end{exe}

The verb form is conjugated as we expect from section 8.1 on subject person marking. It agrees with the subject of the sentence, which either occurs before the object or only as a verbal suffix (as seen here). However, we don't see any object person agreement.
